...
azecure = LINETCR.LINE() # az (fungsi Akun Utama Ke Kick bisa Terima Undangan dari Bot Otomatis)
azecure.login(token="EnjncS85lQZqhFBjbwR6.1KAJPWzm3lsd3XhagT+FvG.bSyaJaV7+Yh/EnFTITJxgNaKgNcagtmL9fDkMl/ODc8=")
#azecure.login(qr=True)
azecure.loginResult()

cl = LINETCR.LINE() #x1
cl.login(token="EnAKoYaPKx6rYzgHRUEa.KMu+wB1O/CwcCXXEWFTykG.xyo0LAtwvDkUOjKz3dMI8n1BE38Vl3aFm6DnKyRPgT8=")
#cl.login(qr=True)
cl.loginResult()

ki = LINETCR.LINE() #x2
ki.login(token="EnMfczn4arySYXEVeEs1.cSsJLerQeQq8ELgJnnDFCq.KAChq12wKPm0VAqHfCkz8crF561fRZzH7lPgwNSwsac=")
#ki.login(qr=True)
ki.loginResult()

kk = LINETCR.LINE() #x3
kk.login(token="Enk4Z0Kk7W26EZiciOm2.QCd5+kNRscZCogRtx+xzaG.v3kEbR49FU7j0aFLs1htxD8PoG8SO5vrW6+T6NFoLzU=")
#kk.login(qr=True)
kk.loginResult()

k1 = LINETCR.LINE() #x4 Backup 
k1.login(token="En6mDKjAaXIckSBGjq31.Sjcr0qgLooEZ5Upv24W08q.r71BVf9RNnP3kE5V2xkfysF+mqWyWv1tZZLJYLaS0Ug=")
#k1.login(qr=True)
k1.loginResult()

print "[Command]Login success"
reload(sys)
...
ᵗʰᵃⁿᵏˢ ᵗᵒ : 
ᵖᵃʳᵃᴸᴵᴺᴱ ᴮᵒᵗ ᴰᵉᵛᵉˡᵒᵖᵉʳ
ᵖᵃʳᵃᴸᴵᴺᴱ ʰᵃᶜᵏᴮᵒᵗ  ᴰᵉᵛᵉˡᵒᵖᵉʳ
╠•Minat/Kripik/saran/curhat/resep masakan/ :
║ ᶜᵖ.
╚ [belum ada nama]"""  
...
admin=["ud646a3a7064cdcb483df11c01adfdf56","u370d300e76e607da9ba4eed2870befe8","u5b8fc9bdc096c726383c6f99c7265a55","u5c867b91afa94b934acc3ef4c567fcb4","udf545ae51078c1871518823d32855989","u13baee5b3066dd15868a242b9b31cb35","udee8ee9a226e2fbbfea122e50dab8f43","ucf11ea078a2a331f879f0176803983a6","u6d55a2a061b3edfd95b7e8144d8a25eb","ude02604ec5cb395174d2bad657deb8c0"] 
...
#--------------------------------
            ...
            if msg.contentType == 13:
               if wait["wblack"] == True:
                   ...
                   if msg.contentMetadata["mid"] in wait["blacklist"]:
                        ...
                        ki.sendText(msg.to,"It is not in the blacklist")
                        kk.sendText(msg.to,"It is not in the blacklist")
                ...
                if wait["lang"] == "JP":
                    cl.sendText(msg.to,aboutMessage)
                else:
                    cl.sendText(msg.to,aboutMessage)
            elif msg.text in ["Protect list"]:
		...
		gs = kk.getGroup(msg.to)
                ...
                if targets == []:
                   ...
                   for target in targets:
                        try:
                            owner.append(target)
                            cl.sendText(msg.to,"Owner has been added")
                        except:
                            pass
	...
	#============add owner finish========  
              ...
              if msg.from_ in owner:
                ...
                if targets == []:
                   ...
                   for target in targets:
                        try:
                            admin.remove(target)
                            cl.sendText(msg.to,"Owner has been deleted")
                        except:
                            pass
                print "[Command]Admin remove executed"
              else:
              ...
              if owner == []:
                  cl.sendText(msg.to,"Owner list is empty")
              else:
                  cl.sendText(msg.to,"Please wait...")
                  mc2 = "╔ [Owner bot protect]\n╠══════════════\n"
                  for mi_d in owner:
                      mc2 += "╠•" +cl.getContact(mi_d).displayName + "\n"
		  cl.sendText(msg.to,mc2)
    ...
    #-----------------=Selesai=------------------
            ...
            elif msg.text in ["Cancel invite2"]:
              if msg.from_ in admin:
                if msg.toType == 2:
		...
		for ticket_id in n_links:
            ...
            elif msg.text in ["Renamex1 "]:
              if msg.from_ in admin:
                string = msg.text.replace("Renamex1 ","")
                if len(string.decode('utf-8')) <= 20:
                    profile = cl.getProfile()
                    profile.displayName = string
                    cl.updateProfile(profile)
                    cl.sendText(msg.to,"rename to " + string + ", Done")
            elif msg.text in ["Renamex2 "]:
              if msg.from_ in admin:
                string = msg.text.replace("Renamex2 ","")
                if len(string.decode('utf-8')) <= 20:
                    profile_B = ki.getProfile()
                    profile_B.displayName = string
                    ki.updateProfile(profile_B)
                    ki.sendText(msg.to,"rename to " + string + ", Done")
            elif msg.text in ["Renamex3 "]:
              if msg.from_ in admin:
                string = msg.text.replace("Renamex3 ","")
                if len(string.decode('utf-8')) <= 20:
                    profile_B = kk.getProfile()
                    profile_B.displayName = string
                    kk.updateProfile(profile_B)
                    kk.sendText(msg.to,"rename to " + string + ",Done")
            elif msg.text in ["Mc "]:
              if msg.from_ in admin:
            ...
            elif msg.text in ["Auto cancel on"]: # tiap ada yg invite auto cancel
              if msg.from_ in admin:
                if wait["Protectcancl"] == True:
                    if wait["lang"] == "JP":
            ...
            elif msg.text in ["Set gc"]:
              if msg.from_ in admin:
                md = "╔ [Status Protect]\n╠═════════════\n"
                if wait["Protectgr"] == True: md+="╠•Protect QR 「on」\n"
                else: md+="•╠Protect QR 「off」\n"
                if wait["Protectcancl"] == True: md+="╠•Auto cancel 「on」\n"
		...
		if wait["autoCancel"]["on"] == True:md+="╠•Group Cancel\n╚═════════════" +str(wait["autoCancel"]["members"])+ "\n"
            ...
            elif msg.text in ["Cancel all invite"]:
              if msg.from_ in admin:
              ...
              if msg.from_ in admin:
                if wait["autoAdd"] == False:
                    ...
                    wait["autoAdd"] = False
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"Auto add is Off")
                    else:
                        cl.sendText(msg.to,"è¦�äº†å…³æ–­ã€‚")
...
#--------===---====--------------
        ...
        #-------------Fungsi Jam on/off Start-------------------#            
              ...
              if msg.from_ in admin:
                if wait["clock"] == True:
                    kk.sendText(msg.to,"3 hours stay  n On")
                else:
                    wait["clock"] = True
                    now2 = datetime.now()
              ...
              if msg.from_ in admin:
                if wait["clock"] == False:
                    kk.sendText(msg.to,"3 hours stay n Off")
                else:
                    wait["clock"] = False
        ...
        #-------------Jam Update Finish-------------------#
             ...
             # if msg.from_ in admin:
                cl.sendText(msg.to, "Set read point...")
                try:
                ...
                wait2['setTime'][msg.to] = datetime.strftime(now2,"「%d/%m/%y•%H:%M」")
                wait2['ROM'][msg.to] = {}
                print wait2
            elif msg.text == "View":
                 #if msg.from_ in admin:
                    if msg.to in wait2['readPoint']:
                        if wait2["ROM"][msg.to].items() == []:
                            ...
                            for rom in wait2["ROM"][msg.to].items():
                                print rom
                                chiya += rom[1] + "\n"
                        cl.sendText(msg.to, "╔[is Online]\n╠═════════════%s\n╚═════════════\n╔[is Siders]\n╠═════════════\n%s╠═════════════\n╚%s" % (wait2['readMember'][msg.to],chiya,setTime[msg.to]))
                    else:
                        cl.sendText(msg.to, "Please Type command [Set] to reset read Point \nand then Type Command [View]\nThank You")
#-----------------------------------------------
...
#-----------------------------------------------
    ...
    #------------- Tag All Finish---------------#
            elif msg.text in ["Like owner"]: #Semua Bot Ngelike Status Akun Utama
              if msg.from_ in owner:
        ...
        #----------------Kick User Target Start----------------------#
            elif "Km " in msg.text:
              if msg.from_ in admin:
                nk0 = msg.text.replace("Km ","")
                nk1 = nk0.lstrip()
                nk2 = nk1.replace("@","")
                nk3 = nk2.rstrip()
                _name = nk3
        ...
        #----------------Kick User Target Finish----------------------#      
            ...
            #----------------Banned User Target Start-----------------------#
            elif "Banned @" in msg.text:
              if msg.from_ in admin:
                if msg.toType == 2:
                    print "[Command] Banned member Success"
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                                ...
                                random.choice(KAC).sendText(msg.to,"Error")
            #----------------Banned User Target Finish-----------------------# 
            #----------------Mid via Tag--------------
            elif "Mid @" in msg.text:
              if msg.from_ in owner:
		print "[Command] cek mid via tag"    		 
                ...
                for g in gs.members:
                    if _nametarget == g.displayName:	   
                       random.choice(KAC).sendText(msg.to, g.mid)		    
                        ...
                        pass
            #-----------------------------------------
            #----------------Unbanned User Target Start-----------------------#
            elif "Unban @" in msg.text:
              if msg.from_ in admin:
                if msg.toType == 2:
                    print "[Command] Unbaned via tag "
      ...
      #--------------Fungsi Broadcast Finish-----------#
            ...
            elif msg.text in ["List gid"]: #Melihat List Group + ID Groupnya (Gunanya Untuk Perintah InviteMe:)
              if msg.from_ in owner:
                gid = cl.getGroupIdsJoined()
                h = ""
                for i in gid:
      ...
      #--------------List Group------------
       #------------ Keluar Dari Semua Group------
            elif msg.text in ["Left all group"]: # Keluar Dari Semua Group Yang Di dalem nya  ada bot(Kalo Bot Kalian Nyangkut di Group lain :D)
              if msg.from_ in owner:
                 gid = cl.getGroupIdsJoined()
...
#-----------------------------------------------
       ...
       #-------------Respon Start---------------------#
              ...
              if msg.from_ in admin:
                ...
                cl.sendText(msg.to,"already Protection")
      #-------------Respon Finish---------------------#
      ...
      #-------------Fungsi Chat ----------------
                 ...
                 quote = ['siap Bozz','Jelek lu','cakep amat','Ah Kamvret Lu','Muke gileeee','Ada Orang gak disini']
...
#---------------------
        if op.type == 17:
           ...
           random.choice(KAC).sendText(op.param1, "Welcome to " + str(ginfo.name)+" Group"+"\n"+"Creator " + str(ginfo.name) +" Group is"+ ":\n" + ginfo.creator.displayName)
           random.choice(KAC).sendText(op.param1,"Chat and have fun, anggap aja rumah sendiri ,,ahhaaayyyy")
           print "Member has join the group"
        if op.type == 15:
           if op.param2 in Bots:
              return          
	   quote = ['nah loohh knp tuh left','yaaaaahh,,km left, aku di tinggalin','waduuuuh ada yg left','ada yg left lagi']
           psn1 = random.choice(quote)
	   random.choice(KAC).sendText(op.param1,psn1)
           #print cl.getGroup(op.param1)
	   print "Member has leave the group"
#------------------------
        if op.type == 59:
            print op
    ...
    while True:
        ...
        #while a2():
            ...
            if wait["clock"] == True:
                now2 = datetime.now()
                nowT = datetime.strftime(now2,"(%H:%M)")
		profile= azecure.getProfile()
                profile.displayName = wait["cName"]
                azecure.updateProfile(profile)
                ...
                profile2.displayName = wait["cName3"]
                ki.updateProfile(profile2)

                profile3 = kk.getProfile()
                profile3.displayName = wait["cName4"]
                kk.updateProfile(profile3)
                
                profile4 = k1.getProfile()
                profile4.displayName = wait["cName5"]
                k1.updateProfile(profile4)
            time.sleep(600)
        except:
            pass
