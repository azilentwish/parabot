...
#--------------------------------
    ...
    #-----------------=Selesai=------------------
              ...
              if msg.from_ in admin:
                if msg.toType == 2:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        random.choice(KAC).sendText(msg.to,"already Closed")
                else:
                    if wait["lang"] == "JP":
                        random.choice(KAC).sendText(msg.to,"Can not be used outside the group")
                    else:
		...
		for ticket_id in n_links:
            ...
            elif msg.text in ["å¼·åˆ¶è‡ªå‹•é€€å‡º:ã‚ªãƒ³","Leave on","Auto left on","å¼·åˆ¶è‡ªå‹•é€€å‡ºï¼šé–‹"]:
              if msg.from_ in admin:
                if wait["leaveRoom"] == True:
                    if wait["lang"] == "JP":
                        cl.sendText(msg.to,"already on")
            ...
            elif msg.text in ["å¼·åˆ¶è‡ªå‹•é€€å‡º:ã‚ªãƒ•","Leave off","Auto left off","å¼·åˆ¶è‡ªå‹•é€€å‡ºï¼šé—œ"]:
              if msg.from_ in admin:
...
#-----------------------------------------------
    ...
    #-------------Fungsi Tag All Finish---------------#
              ...
              if msg.from_ in admin:
                if msg.toType == 2:
                    ...
                    if matched_list == []:
                        ...
                        random.choice(KAC).sendText(msg.to,"Please Don't Come Back OKey􀨁􀆷devil smile􏿿")
                        return
      ...
      #--------------Fungsi Broadcast Finish-----------#
            ...
            elif msg.text in ["Lg"]: #cek List Group
              if msg.from_ in admin:
      ...
      #--------------List Group------------
       #------------ Keluar Dari Semua Group------
            elif msg.text in ["Bot out all group"]: # parabot left Dari Semua Group Yang Di dalem nya  ada bot(Kalo ada Bot Nyangkut di Group lain)
              if msg.from_ in owner:
                 gid = cl.getGroupIdsJoined()
...
#-----------------------------------------------
       ...
       #-------------Fungsi Respon Start---------------------#
              ...
              if msg.from_ in admin:
                cl.sendText(msg.to,"Hadir..√.۱")
                ki.sendText(msg.to,"Hadir...√.۲")
                kk.sendText(msg.to,"Hadir....√.۳")                
